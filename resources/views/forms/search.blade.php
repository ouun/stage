<form role="search" method="get" class="relative flex items-stretch w-full h-full p-0 m-0 overflow-hidden border search-form max-h-12 border-inherit text-copy" action="{{ home_url('/') }}">
  ...
  <input id="search" type="search" class="w-full py-0 pl-6 pr-3 m-0 leading-none bg-transparent border-0 appearance-none search-field h-inherit focus:outline-none" placeholder="{!! __('Search &hellip;', 'stage') !!}" value="{!! get_search_query() !!}" name="s">
